...
| F7 | Semua scene | `Directional Light`, RenderSettings, `Assets/Settings/PC_RPAsset.asset` + `Mobile_RPAsset.asset`, 56 komponen ApiFlare, `Warm_Spotlight_1/2`, `LampuShell_S4` | Kegelapan berlapis: (a) **tidak ada baked GI** — `LightingData.asset` tidak ada, static flags hanya Batching (tanpa ContributeGI) → semua realtime; (b) Directional 0.05; (c) ambient Flat sangat gelap + skybox null; (d) **limit 4 lampu/objek** di kedua RP asset padahal ±60 lampu kecil realtime → tambalan gelap; (e) **56 komponen Missing Script** — scene menunjuk guid ApiFlare lama `474bcb49…` (dangling) vs guid file sekarang `883bd2fa…` → flare/flicker api mati; (f) spotlight display teddy S1 off + intensity 0; (g) `LampuShell_S4` 1.0 (yang lain 1.4–3.0); (h) `Assets/Generated/GEN_VolumeProfile.asset` **kosong** (`components: [{fileID: 0}]`) → Bloom hilang. | Terapkan: guid-swap ApiFlare (56 ref), Directional →0.18, AmbientSky ×1.6, Warm_Spotlight on+2.0, LampuShell_S4 →1.4, limit lampu 4→8 (PC & Mobile). Bloom & bake = Manual §1 & §4. | blocker | ✅ commit `267bb9d` |
...
- GUID "dangling" TMP yang dilaporkan alat audit = false alarm (resolve ke package). Dangling yang nyata hanya ApiFlare (F7) + sisa guid lama di `Assets/Models/Floreswa/Scene/SampleScene.unity` (scene demo, di luar scope — abaikan atau hapus foldernya sekalian).
...
1. Buka scene `UAS_Main` → **Console tidak boleh ada error baru** (cari "Unable to parse" / "missing script" — harusnya justru 56 warning missing script *hilang*).
